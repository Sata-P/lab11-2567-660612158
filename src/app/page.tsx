  ...
  return (
    <div className="mx-auto vstack gap-3" style={{ width: "400px" }}>
      ...
      <div>
        ...
        <div>
          <input
            className={"me-2 form-check-input" + (genderError ? " is-valid" : "")}
            type="radio"
          ...
          <div>{genderError ? <div className="text-danger ">Please select gender</div> : ""}</div>
        </div>
      </div>
      ...
      <div>
        ...
        <div>{(buyBottle && buyShoes && buyCap) ? <span className="text-success d-block">(20% Discounted)</span> : ""}</div>
      </div>

      {/* Terms and conditions */}
      <div>
        <input className="me-2" type="checkbox" onChange={UserAgreedOnChange} checked={isUserAgreed}/>I agree to the terms and
        conditions
      </div>
      ...
      <button
        ...
        disabled={isUserAgreed ? false : true}
      >
        Register
      </button>
    </div>
